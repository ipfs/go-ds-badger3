...
func TestNotFounds(t *testing.T) {
	...
	if err != ds.ErrNotFound {
		t.Fatal("expected ErrNotFound for key that doesn't exist, got: ", err)
	}
...
func TestClosedError(t *testing.T) {
	...
	err = d.PutWithTTL(bg, key, nil, time.Second)
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}

	err = d.SetTTL(bg, key, time.Second)
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}

	_, err = d.GetExpiration(bg, ds.Key{})
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}
	...
	err = tx.PutWithTTL(bg, key, nil, time.Second)
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}

	_, err = tx.GetExpiration(bg, key)
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}

	err = tx.SetTTL(bg, key, time.Second)
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}
	...
	_, err = tx.GetSize(bg, key)
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}

	err = tx.Delete(bg, key)
	if !errors.Is(err, ErrClosed) {
		t.Error(errMsg, err)
	}
